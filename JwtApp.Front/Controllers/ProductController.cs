    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            ...
            if (token != null)
            {
                ...
                var response = await client.GetAsync("http://localhost:5147/api/products");
        ...
        public async Task<IActionResult> Remove(int id)
        {
            ...
            if (token != null)
            {
                ...
                await client.DeleteAsync($"http://localhost:5147/api/products/{id}");
            }
        ...
        public async Task<IActionResult> Create()
        {
            ...
            //model.Categories = new Microsoft.AspNetCore.Mvc.Rendering.SelectList();
            return RedirectToAction("List");
        }
        [HttpPost]
        public async Task<IActionResult> Create(CreateProductModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var response = await client.PostAsync("http://localhost:5147/api/products", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            if (token != null)
            {
                ...
                var responseProduct = await client.GetAsync($"http://localhost:5147/api/products/{id}");
                ...
                if (responseProduct.IsSuccessStatusCode)
                {
                    ...
                    var responseCategory = await client.GetAsync($"http://localhost:5147/api/categories");
        ...
        public async Task<IActionResult> Update(UpdateProductModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (token != null)
                {
                    ...
                    var jsonData = JsonSerializer.Serialize(model);
                    var content = new StringContent(jsonData, Encoding.UTF8, "application/json");
                    var response = await client.PutAsync("http://localhost:5147/api/products", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("List");
                    }
